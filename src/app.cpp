...
App::App(std::span<const std::string_view> args) {
	...
	m_device->create_swapchain(handle);
...
void App::run() {
	...
	while (m_running) {
		...
		state.setViewport(nvrhi::ViewportState().addViewportAndScissorRect(nvrhi::Viewport(fbinfo.width, fbinfo.height)));
		...
		m_command_list->drawIndexed(nvrhi::DrawArguments().setVertexCount(m_indices.size()));
